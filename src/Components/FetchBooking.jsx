...
export default function FetchBooking({
  ...
  const showNotFoundModal = (overrideMode) => {
  ...
  const fetchRows = async (queryOverride, modeOverride) => {
    ...
    const payloadBase = { action: "search", mode: searchMode, query: qStr };
    ...
    if (!rows.length && searchMode === "vehicle") {
  ...
  const runSearch = async ({ modeOverride, queryOverride, silent } = {}) => {
  ...
  useEffect(() => {
    ...
    runSearch({ modeOverride: nextMode, queryOverride: queryValue, silent: true });
